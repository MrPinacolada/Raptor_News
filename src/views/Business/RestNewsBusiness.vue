<template>
  <div class="subNewsConteiner">
    <article class="subTitle" v-for="art in CurrentArt">
      <picture>
        <RouterLink :to="{ name: 'BusinessArts', params: { id: art.id } }">
...
export default defineComponent({
  ...
  setup() {
    ...
    CurrentArt.value = Store().$state.BusinessARTS.filter((item: any) => {
      return item.title !== Store().$state.DailyArtBusinessPage;
    });
